...
import importlib
from utils.fmodule import powerlaw, random_split, gaussian_mech_vec
import copy
...
class BasicTaskGen:
    ...
    def __init__(self, benchmark, dist_id, skewness, rawdata_path, seed=222):
        ...
        self.num_clients = -1
        # self.epsilon = -1
        # self.level_n_system = 0.8  # p
        # self.level_n_lowerb = 0.5  # T
        self.seed = seed
    ...
    def get_taskname(self):
        ...
        # taskname = '_'.join([self.benchmark, 'cnum' +  str(self.num_clients),'dist' + str(self.dist_id), 'skew' + str(self.skewness).replace(" ", ""), 'level_n_system' + str(self.level_n_system), 'level_n_lowerb' + str(self.level_n_lowerb),'seed'+str(self.seed)])
    ...
    def create_task_directories(self):
        ...
        os.mkdir(taskpath)                          
        os.mkdir(os.path.join(taskpath, 'record'))      #os.mkdir()创建目录

    def _check_task_exist(self):
        """Check whether the task already exists."""
        taskname = self.get_taskname()
        return os.path.exists(os.path.join(self.rootpath, taskname))  # os.path.exists判断括号里的文件是否存在
...
class DefaultTaskGen(BasicTaskGen):
    ...
    def partition(self):
        ...
        if self.dist_id == 0:
            """IID"""
            d_idxs = np.random.permutation(2000)
            local_datas = np.array_split(d_idxs, self.num_clients)

        elif self.dist_id == 18:
            """用户数据逐渐增加powerlaw"""
            #用户的数据量为range(x)/self.num_clients
            train_sum = int(self.num_clients) * 2000
            local_datas = powerlaw(list(range(train_sum)), self.num_clients)       

        elif self.dist_id == 19:
            """类别不均匀classimbalance"""
            n_classes = 26              #标签的类别
            sample_size_cap = int(self.num_clients) * 2000     #sample_size_cap//self.num_clients为用户的数据量
            data_indices = [torch.nonzero(torch.tensor(self.train_data.targets) == class_id).view(-1).tolist() for class_id in range(1,n_classes+1)]

            # class_sizes = np.linspace(1, n_classes, self.num_clients, dtype='int')
            class_sizes = np.linspace(1, n_classes, self.num_clients, dtype='int')
            print("class_sizes for each party", class_sizes)
            party_mean = sample_size_cap // self.num_clients

            from collections import defaultdict

            party_indices = defaultdict(list)
            for party_id, class_sz in enumerate(class_sizes):
                classes = range(0, class_sz) # can customize classes for each party rather than just listing
            ...
            local_datas = [party_index_list for party_id, party_index_list in party_indices.items()]   

        elif self.dist_id == 20:
            #用户一样 uniform
            ...
            n_participants = self.num_clients
            n_classes = 26
            sample_size_cap = int(self.num_clients) * 4000      #sample_size_cap//self.num_clients为用户的数据量
            data_indices = [torch.nonzero(torch.tensor(self.train_data.targets) == class_id).view(-1).tolist() for class_id in range(1, 1+n_classes)]
            # class_sizes = np.linspace(0, 26, self.num_clients, dtype='int')
            class_sizes = np.linspace(0, 25, 26, dtype='int')
            print("label_type for each party", class_sizes)
            party_mean = (sample_size_cap // self.num_clients)//13

            from collections import defaultdict
            party_indices = defaultdict(list)
            
            for i in range(10):
                class_sizes = np.random.permutation(class_sizes)
                for party_id, class_sz in enumerate(class_sizes):
                    if party_id < 13:
                        selected_indices = random.choices(data_indices[class_sz], k = party_mean)
                        party_indices[i].extend(selected_indices)
            
            local_datas = [party_index_list for party_id, party_index_list in party_indices.items()]


        elif self.dist_id == 22:

            n_participants = self.num_clients
            n_classes = 26
            sample_size_cap = int(self.num_clients) * 4000      
            # data_indices = [torch.nonzero(torch.tensor(self.train_data.targets) == class_id).view(-1).tolist() for class_id in range(n_classes)]
            data_indices = [torch.nonzero(torch.tensor(self.train_data.targets) == class_id).view(-1).tolist() for class_id in range(1, 1+n_classes)]
            # class_sizes = np.linspace(0, 26, self.num_clients, dtype='int')
            class_sizes = np.linspace(0, 25, 26, dtype='int')
            print("label_type for each party", class_sizes)
            party_mean = (sample_size_cap // self.num_clients)//5
...
class ClassifyCalculator(BasicTaskCalculator):
    ...
    def data_to_device(self, data):
        return data[0].float().to(self.device), data[1].to(self.device)
...
class XYTaskReader(BasicTaskReader):
    ...
    def read_data(self):
        ...
        validation = XYDataset(feddata['dvalid']['x'], feddata['dvalid']['y'])
        ...
        return train_datas, valid_datas, validation,test_data, feddata['client_names']
